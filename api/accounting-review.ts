...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (error) {
      console.error("ACCOUNTING_REVIEW_LIST_FAILED", { message: error.message, details: error.details, hint: error.hint, code: error.code });
      throw new HttpError(500, "ACCOUNTING_REVIEW_LIST_FAILED", "Unable to query accounting review items");
    }
