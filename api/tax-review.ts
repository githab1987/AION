...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (error) {
      console.error("TAX_REVIEW_LIST_FAILED", { message: error.message, details: error.details, hint: error.hint, code: error.code });
      throw new HttpError(500, "TAX_REVIEW_LIST_FAILED", "Unable to query tax review items");
    }
