(function () {
  ...
  async function runP2Test() {
    const button = document.getElementById("p2RunButton");
    const message = document.getElementById("p2TestMessage");
    const summary = document.getElementById("p2Summary");
    const resultsContainer = document.getElementById("p2Results");

    button.disabled = true;
    button.textContent = "Menjalankan...";

    message.textContent = "Memverifikasi authorization middleware...";
    summary.innerHTML = "";
    resultsContainer.innerHTML = "";

    try {
      /*
       * Reuse the application's existing apiRequest().
       *
       * This means token + workspace headers are handled by the
       * existing authenticated frontend flow.
       *
       * The token is never rendered, logged, or sent to this UI code
       * as a displayed value.
       */
      if (typeof apiRequest !== "function") {
        throw new Error("Application API client is not available.");
      }
      ...
      if (!response || !response.results) {
        throw new Error("P2 test returned an invalid response.");
      }
      ...
      /*
       * Never display the access token or raw authorization headers.
       */
      const safeMessage =
      ...
      summary.innerHTML = `
        <strong>P2 tidak selesai.</strong><br>
        Periksa deployment Vercel dan endpoint /api/v1/p2-test.
      `;
    } finally {
      button.disabled = false;
      button.textContent = "Jalankan P2 Test";
    }
  }
  ...
  function waitForApplication() {
    /*
     * app.js is loaded separately. We wait until the application has
     * initialized before exposing the P2 launcher.
     */
    let attempts = 0;
